...
function setCookie(name: string, value: string, options: any = {}) {
  ...
  // ✅ Normalize SameSite
  const sameSiteRaw = String(opts.sameSite || "lax").toLowerCase();
  const sameSite =
    sameSiteRaw === "none"
      ? "None"
      : sameSiteRaw === "strict"
  ...
  if (opts.expires)
    cookieStr += `; Expires=${new Date(opts.expires).toUTCString()}`;

  cookieStr += `; Path=${opts.path || "/"}`;
  if (opts.domain) cookieStr += `; Domain=${opts.domain}`;

  cookieStr += `; SameSite=${sameSite}`;
  if (mustSecure) cookieStr += `; Secure`;
...
export default function LoginPage() {
  ...
  // ✅ Supabase browser client that stores PKCE verifier in COOKIES
