...
import { Mail, Lock, ArrowRight, Loader2, User, Zap, Users, Share2, Star, Globe, ArrowLeft } from "lucide-react";
...
export default function LoginPage() {
  ...
  // 👇👇👇 FIXED GOOGLE LOGIN (100% WORKING) 👇👇👇
  const handleGoogleLogin = async () => {
    setLoading(true);
    
    // 🔥 SAFE URL LOGIC:
    // Localhost par zabardasti "http://localhost:3000" use karenge
    // Live par automatic window.location.origin use karenge
    const redirectUrl = process.env.NODE_ENV === 'development' 
        ? "http://localhost:3000/auth/callback" 
        : `${window.location.origin}/auth/callback`;
    
    console.log("Redirecting to:", redirectUrl); // Console mein check kar sakte ho
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Same logic for Email Signup
    const redirectUrl = process.env.NODE_ENV === 'development' 
        ? "http://localhost:3000/auth/callback" 
        : `${window.location.origin}/auth/callback`;

    if (isSignUp) {
      if (!fullName.trim()) { toast.error(t.toast_name); setLoading(false); return; }
    ...
    <div className="min-h-screen flex bg-[#050505] text-white font-sans relative">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#0a0a0a] border-r border-white/5 flex-col justify-center px-16 relative overflow-hidden">
        ...
        <div className="relative z-10">
            <h2 className="text-4xl font-bold mb-12 leading-tight">{t.hero_title} <br/> <span className="text-purple-400">{t.hero_highlight}</span></h2>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative">
        <div className="max-w-md w-full">
            ...
            {/* 🔵 ONLY GOOGLE BUTTON (Full Width) */}
            <div className="mb-8">
                 <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white text-black py-3.5 rounded-xl font-bold hover:bg-gray-100 transition-all active:scale-95 shadow-lg shadow-white/5 text-base">
                    {/* Official Google SVG */}
            ...
            <div className="relative mb-8">
                ...
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#050505] px-2 text-gray-500">{t.or_email}</span></div>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
                {isSignUp && (
                    <div className="relative group animate-in fade-in slide-in-from-top-2">
                        ...
                        <input type="text" placeholder={t.placeholder_name} className="w-full bg-[#111] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-purple-500 focus:outline-none transition-all"
                ...
                <div className="relative group">
                    ...
                    <input type="email" placeholder={t.placeholder_email} className="w-full bg-[#111] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-purple-500 focus:outline-none transition-all"
                        value={email} onChange={(e) => setEmail(e.target.value)} required />
                </div>

                <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-purple-400 transition-colors" />
                    <input type="password" placeholder={t.placeholder_pass} className="w-full bg-[#111] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-purple-500 focus:outline-none transition-all"
                        value={password} onChange={(e) => setPassword(e.target.value)} required />
                </div>

                <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-purple-900/25">
                    {loading ? <Loader2 className="animate-spin" /> : (isSignUp ? t.btn_signup : t.btn_login)} <ArrowRight className="w-4 h-4" />
                </button>
            </form>

            <p className="mt-8 text-center text-sm text-gray-500">
                {isSignUp ? t.have_account + " " : t.need_account + " "}
                <button onClick={() => { setIsSignUp(!isSignUp); setFullName(""); }} className="text-purple-400 hover:text-white font-bold transition-colors">
