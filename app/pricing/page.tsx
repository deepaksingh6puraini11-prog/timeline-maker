    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-purple-500/30 overflow-hidden relative">
      ...
      <nav className="absolute top-0 left-0 w-full px-6 py-6 flex justify-between items-center z-50">
          ...
          <button 
              ...
              className="flex items-center gap-1 text-sm font-bold text-gray-400 hover:text-white transition-colors border border-white/10 px-3 py-1.5 rounded-full hover:bg-white/5 backdrop-blur-sm"
          >
              <Globe size={14} />
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid md:grid-cols-3 gap-8 items-start">
            ...
            {/* 2️⃣ SINGLE PROJECT (HIGHLIGHTED) */}
            <div className="relative z-10">
                ...
                <PricingCard 
                    ...
                    price={t.single_price} 
                    period={t.single_period}
                    desc={t.single_desc}
                    features={t.single_features}
                    btnText={t.single_btn}
                    onClick={() => handlePurchase('single')}
                    loading={loading === 'single'}
                    highlight={true}
                    badge={t.single_badge}
                    delay={0.4}
                />
            </div>
            ...
            <PricingCard 
                ...
                desc={t.monthly_desc}
                features={t.monthly_features}
                btnText={t.monthly_btn}
        ...
        <motion.div 
            ...
            className="mt-16 text-center flex items-center justify-center gap-6 text-gray-500 text-sm"
        >
...
// 🧩 Helper Component for Card Design
function PricingCard({ title, price, period, desc, features, btnText, onClick, loading, highlight, badge, delay }: any) {
    return (
        <motion.div 
            ...
            className={`relative bg-[#0f1014] border p-8 rounded-2xl flex flex-col h-full transition-all duration-300 group hover:-translate-y-2
                ${highlight ? 'border-purple-500/50 shadow-2xl shadow-purple-900/20' : 'border-white/10 hover:border-white/20'}
            `}
        >
            {badge && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-lg whitespace-nowrap">
                    {badge}
                </div>
            )}
            ...
            >
                {loading ? (
                    ...
                    <>
                        {highlight && <Zap size={16} fill="currentColor" />}
                        {btnText}
                    </>
                )}
            </button>
